...
mem_init(void)
{
  ...
  // Your code goes here:
  boot_map_region(kern_pgdir, UPAGES, ROUNDUP((sizeof(struct PageInfo) * npages), PGSIZE), PADDR(pages), (PTE_W | PTE_P));
  ...
  // Map user program
  //
  /*boot_map_region(kern_pgdir, (uint32_t)_binary_userinit_start, ROUNDUP((uint32_t)_binary_userinit_size, PGSIZE), PADDR((uint32_t)_binary_userinit_start), PTE_W | PTE_U);*/
...
page_alloc(int alloc_flags)
{
  ...
  if (ret != NULL)
  {
    ...
    if (alloc_flags & ALLOC_ZERO)
      memset((void *)(page2kva(ret)), 0, PGSIZE); // should be va not pa
    num_free_pages--;
  }
  return ret;
}
...
page_free(struct PageInfo *pp)
{
  ...
  if (pp)
  {
    if (pp->pp_ref != 0)
    {
      panic("page_free: Reference count for current page is not zero\n");
      return;
    }
    if (pp->pp_link != NULL)
    {
      panic("page_free: pp_link has a non-NULL value\n");
      return;
    }
...
{
  // Fill this function in
  pde_t* pCurrPDE = pgdir + PDX(va);
  pde_t currPDE = *pCurrPDE;
  pte_t* pgtable;

  // first level
  if (currPDE & PTE_P)
  {
    pgtable = (pte_t*)(KADDR(PTE_ADDR(currPDE)));
  }
  else
  {
    if (create == 0)
      return NULL;
    else // Create page table (actually a page)
    {
      physaddr_t physaddr;
      ...
      physaddr = page2pa(pgInfo);
      pgtable = KADDR(physaddr);

      pgInfo->pp_ref ++;
      *pCurrPDE = (physaddr | PTE_P | PTE_U | PTE_W); // why?
    }
  }
...
tlb_invalidate(pde_t *pgdir, void *va)
{
  ...
  void
setupuvm(pde_t *pgdir, uint32_t start, uint32_t size)
{
