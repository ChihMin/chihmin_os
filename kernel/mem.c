...
page_init(void)
{
    ...
    /* TODO */
   // extern entry_pgtable[];   
   // entry_pgtable[0] |= PTE_A;  
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
    ...
    /* TODO */
    struct PageInfo * new_page;
    physaddr_t new_page_addr;
    
    if ((~pgdir[PDX(va)] & PTE_P) && create == false) {
        return NULL;
    }
        ...
        new_page_addr = PTE_ADDR(pgdir[PDX(va)]);
    }
    
    return (pte_t *)(KADDR(new_page_addr))+PTX(va);
}
...
boot_map_region(pde_t *pgdir, uintptr_t va, size_t size, physaddr_t pa, int perm)
{
    ...
    for (current_size = 0; current_size < size; current_size += PGSIZE) {
        ...
        if (page_entry != NULL)
            *page_entry = (pa + current_size) | perm | PTE_P;
        else
            panic("pgdir_wall return NULL\n");
    }
}
...
page_remove(pde_t *pgdir, void *va)
{
    ...
    page = page_lookup(pgdir, va, &page_table);
    if (page == NULL)   return ;
...
setupvm(pde_t *pgdir, uint32_t start, uint32_t size)
{
    ...
    //printk("orig : 0x%x, check_va2pa : 0x%x, size : 0x%x\n", start, check_va2pa(pgdir, start), size);
